...
function Historial() {
    ...
    const obtenerEstado = (transaccion) => {
        ...
        if (
            ["rechazada", "fallida", "cancelada", "error"].some(
                (estadoFallido) =>
                    estado.includes(estadoFallido)
            )
        ) {
            return "rechazada";
        }
        ...
        if (
            ["pendiente", "procesando"].some(
                (estadoPendiente) =>
                    estado.includes(estadoPendiente)
            )
        ) {
            return "pendiente";
        }
    ...
    const formatearMonto = (monto, tipo) => {
        return `${tipo === "ingreso" ? "+" : "-"} $${monto.toLocaleString(
            "es-CO"
        )}`;
    };
